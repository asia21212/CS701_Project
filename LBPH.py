...
def load_feature():
    ...
    pickle.dump(X,open('feature_LBPH_augment.dat','wb'))
    pickle.dump(y,open('class_LBPH_augment.dat','wb'))
...
def train_svm(X_train, X_test, y_train, y_test):
    svm_clf = svm.SVC(C=1000000,kernel='rbf', gamma="auto")
...
def getClassifier(ktype):
    if ktype == 0:
        # Polynomial kernal
        return svm.SVC(C=1000000,kernel='poly', degree=1, gamma="auto")
    elif ktype == 1:
        # Radial Basis Function kernal
        return svm.SVC(C=1000000,kernel='rbf', gamma="auto")
    elif ktype == 2:
        # Sigmoid kernal
        return svm.SVC(C=1000000,kernel='sigmoid', gamma="auto")
    elif ktype == 3:
        # Linear kernal
        return svm.SVC(C=1000000,kernel='linear', gamma="auto")

if __name__ == '__main__':
    # load_feature()
    X = pickle.load(open('feature_LBPH_augment.dat', 'rb'))
    y = pickle.load(open('class_LBPH_augment.dat', 'rb'))
    X = np.array(X)
    y = np.array(y)
    print(X.shape)
    print(y.shape)
