...
def load_feature():
    ...
    pickle.dump(X,open('feature_facenet_augment.dat','wb'))
    pickle.dump(y,open('class_facenet_augment.dat','wb'))
...
def load_feature_test():
    ...
    for person_name in os.listdir(people_dir):
        person_dir = os.path.join(people_dir, person_name)
        print(person_name)
        person_dir = os.path.join(person_dir, 'test')
        for img_name in os.listdir(person_dir):
            if img_name.endswith('.jpg') or img_name.endswith('.png'):
                img_path = os.path.join(person_dir, img_name)
                img = cv2.imread(img_path)
...
def getClassifier(ktype):
    if ktype == 0:
        # Polynomial kernal
        return svm.SVC(C=1000 ,kernel='poly', degree=8, gamma="auto")
    elif ktype == 1:
...
if __name__ == '__main__':
    ...
    print(X.shape)
    print(y.shape)
    X_test,y_test = load_feature_test()
